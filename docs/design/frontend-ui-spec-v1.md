...
- **区块**：插件卡片（标题、ID、权限 Pill、状态徽章、配置区、**安装 / 配置 / 使用 / AI 网关试调 / 卸载**、底部面板含「执行反馈 / 操作日志 / AI 计费明细 / AI 路由策略概览 / 快速新增路由策略」）
- **表单约束**：路由策略表单需内置基础校验（空链路、非法 `|` 分隔），并提供“一键填充策略模板”快捷入口
- **提交确认**：策略提交后需展示最近一次提交回显（策略 ID、插件/任务、更新时间）便于运营复核
- **列表效率**：路由策略列表支持关键字筛选（插件/任务/模型）并按更新时间倒序展示
...
| AI 路由策略列表 | GET | `/api/v1/ai/route/policies` | Query: `offset?`,`limit?` | 管理侧分页查看租户级插件/任务路由策略（按更新时间倒序） |
