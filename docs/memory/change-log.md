...
### CHG-027：AI 网关增加配额与用量统计（v1）
- 变更内容：
  - 新增 AI 计量模型：`ai_usage_events`（调用事件）与 `ai_quotas`（租户配额）。
  - `/api/v1/ai/invoke` 每次调用后自动落库记录（units 估算、成功/失败状态）。
  - 新增 `/api/v1/ai/usage/summary` 返回当月配额、已用量、剩余量与调用成功/失败统计。
  - 新增 `/api/v1/ai/quota`（占位）允许按租户更新月度配额。
  - 聊天页新增 AI 用量卡，展示当月使用概览，流程执行后自动刷新。
- 变更原因：将“能调用”推进到“可观测、可控”的阶段，为后续计费扣次与审计打基础。
- 影响范围：后端 AI 路由/模型扩展、测试补充；前端聊天页新增用量展示。

### CHG-028：AI 审计日志（v1）
- 变更内容：
  - 新增审计模型 `ai_audit_logs`，记录插件、任务类型、provider/model、状态码、错误摘要、请求/输出预览。
  - `/api/v1/ai/invoke` 在写用量事件时同步写入一条审计记录。
  - 新增 `/api/v1/ai/audit/logs`（`offset/limit`）用于按租户分页查看最近调用记录。
  - 聊天页新增“最近 AI 审计记录”卡片，默认展示最近 5 条，便于快速定位失败原因。
  - 补充后端测试覆盖审计查询链路。
- 变更原因：补齐 AI 调用可追溯性，降低排障门槛，为后续审计检索与合规导出打基础。
- 影响范围：后端 AI 模型与路由扩展；前端聊天页新增审计可视化。

### CHG-029：AI 多模型路由（v1）
- 变更内容：
  - 新增环境变量 `AI_TASK_MODEL_MAP`（格式：`task:model,task:model`），用于按 `task_type` 路由模型。
  - `invoke_model` 在远程 provider 下按映射命中模型，未命中时回退 `AI_MODEL`。
  - AI 结果中补充路由元信息（provider、route_model），供内部审计写入与展示。
  - 聊天页“最近 AI 审计记录”增加命中模型显示，便于核对路由策略是否生效。
  - 新增测试覆盖任务模型路由命中场景。
- 变更原因：在不引入复杂编排器的前提下，先实现“任务分流到合适模型”的可配置能力。
- 影响范围：后端配置与 AI 网关服务增强；前端审计卡片信息增强。

### CHG-030：AI 路由策略 v2（失败自动降级）
- 变更内容：
  - 扩展 `AI_TASK_MODEL_MAP`：单任务支持候选模型链路（`modelA|modelB`）。
  - 新增 `AI_FALLBACK_MODELS` 全局兜底模型序列，任务链路失败后继续尝试。
  - AI 网关在远程调用失败（HTTP/请求异常）时自动降级到下一候选，直到成功或全部失败。
  - 调用结果补充 `route_chain/route_fallback_used` 元信息，审计日志输出预览包含路由链路。
  - 新增测试覆盖“首选失败 -> 兜底成功”场景。
- 变更原因：提高线上稳定性，减少单模型波动对业务执行链路的影响。
- 影响范围：后端 AI 路由逻辑增强、环境配置项增加；前端无需改接口即可消费增强后的审计信息。

### CHG-031：AI 路由策略 v3（租户策略 + 熔断窗口）
- 变更内容：
  - 新增 `ai_route_policies` 模型与 API：`GET/POST /api/v1/ai/route/policies`。
  - 支持按租户 + 插件 + 任务类型覆盖模型链路（`model_chain`）并设置禁用模型（`disabled_models`）。
  - 新增熔断配置：`AI_ROUTE_BLOCK_WINDOW_SEC`、`AI_ROUTE_BLOCK_THRESHOLD`。
  - 在熔断窗口内，某模型失败次数达到阈值后会被临时从候选链路剔除。
  - 回退过程中每次失败候选会写入失败审计，保证熔断统计可用；最终成功/失败仍保留主审计记录。
  - 新增测试覆盖策略覆盖与熔断生效场景。
...
### CHG-033：AI 计费联动（v2：差异单价 + 明细查询）
- 变更内容：
  - 新增配置 `AI_PLUGIN_UNIT_PRICE_MAP`，支持按 `plugin_id` 覆盖默认 `AI_UNIT_PRICE`。
  - 新增 `ai_billing_records` 模型，记录每次计费结果（units/单价/金额/状态/原因/余额）。
  - `/api/v1/ai/invoke` 计费成功与余额不足两种情况都会落计费流水。
  - 新增 `GET /api/v1/ai/billing/records`（`offset/limit`）用于查看租户级计费明细。
  - 补充测试覆盖插件差异化单价与计费明细查询。
- 变更原因：让计费策略能按插件精细化定价，并提供可核对的账单明细视图。
- 影响范围：后端 AI 计费能力增强；前端/管理侧可基于新接口展示账单流水。

### CHG-034：插件控制台新增 AI 运营面板（v1）
- 变更内容：
  - `dashboard/plugins` 页面新增“AI 计费明细（最近 6 条）”面板，读取 `/api/v1/ai/billing/records`。
  - 同页新增“AI 路由策略概览”面板，读取 `/api/v1/ai/route/policies`。
  - 在插件安装/配置/使用/卸载、AI 网关试调后自动刷新两个面板，确保数据同步。
  - 面板展示聚焦运营核对字段：插件、任务、units、单价、金额、状态、余额，以及策略链路与禁用模型。
- 变更原因：把新增的 AI 路由与计费能力对业务侧可视化，降低排障与对账门槛。
- 影响范围：前端插件控制台交互增强；后端接口复用，无新增后端改动。

### CHG-035：插件控制台新增“快速新增路由策略”入口
- 变更内容：
  - 在 `dashboard/plugins` 新增轻量表单：`plugin_id`、`task_type`、`model_chain`、`disabled_models`。
  - 表单直接调用 `POST /api/v1/ai/route/policies` 创建/覆盖策略，并将结果写入执行反馈与操作日志。
  - 提交后自动刷新“路由策略概览”和“计费明细”面板，保持运营视图一致。
- 变更原因：减少运营同学配置 AI 路由策略的操作路径，提升策略试错效率。
- 影响范围：前端控制台增强；后端接口复用，无额外后端改动。

### CHG-036：路由策略表单可用性增强（校验 + 模板）
- 变更内容：
  - 新增基础输入校验：`model_chain` 不能为空，且禁止非法 `|` 分隔（如 `||`、首尾 `|`）。
  - 新增禁用模型字段基础格式检查，避免错误分隔符导致保存失败。
  - 新增“一键填充策略模板”按钮，快速回填常用示例（翻译任务双模型链路）。
  - 校验失败时在表单内显示即时错误提示，同时保留原输入便于修正。
- 变更原因：把“可配置”提升到“可正确配置”，减少运营误填带来的联调成本。
- 影响范围：前端插件控制台交互增强；后端接口无变更。

### CHG-037：路由策略提交回显
- 变更内容：
  - 路由策略表单提交成功后，页面显示最近一次提交回显卡片。
  - 回显字段包括：策略 ID、插件 ID、任务类型、更新时间。
  - 回显与执行反馈/日志并行展示，便于运营侧确认“是否保存到正确策略”。
- 变更原因：补齐“提交后确认”环节，降低因接口成功但目标策略不符导致的排查成本。
- 影响范围：前端插件控制台增强；后端接口无变更。

### CHG-038：路由策略编辑/删除闭环
- 变更内容：
  - 后端新增 `POST /api/v1/ai/route/policies/delete`（按策略 ID 删除，租户隔离校验）。
  - 控制台“路由策略概览”列表每条新增“编辑/删除”按钮：
    - 编辑：将策略回填到快速表单，便于覆盖更新。
    - 删除：调用 delete 接口并刷新策略面板。
  - 新增后端测试覆盖策略删除链路。
- 变更原因：将路由策略从“可新增”提升到“可维护（改/删）”，降低长期运营成本。
- 影响范围：后端 AI 路由策略管理接口增强；前端控制台操作链路增强。

### CHG-039：路由策略列表筛选与排序
- 变更内容：
  - 控制台“路由策略概览”新增关键字筛选输入框（匹配插件、任务、模型链路、禁用模型）。
  - 列表默认按 `updated_at` 倒序排序，最新策略优先展示。
  - 无匹配项时给出明确提示，避免误判“无数据”。
- 变更原因：策略数量上来后，先补“找得到”能力，降低运营定位成本。
- 影响范围：前端控制台交互增强；后端接口无改动。

### CHG-040：路由策略批量删除
- 变更内容：
  - 后端新增 `POST /api/v1/ai/route/policies/delete-batch`，支持按 `ids[]` 批量删除（含租户隔离）。
  - 控制台“路由策略概览”新增多选框与“批量删除”按钮，支持一次清理多条策略。
  - 面板刷新时自动清理无效选中项，避免“已删策略仍选中”的状态错乱。
  - 新增后端测试覆盖批量删除链路。
- 变更原因：提升运营批量清理策略效率，减少重复点击操作。
- 影响范围：后端路由策略管理接口增强；前端控制台操作效率增强。

### CHG-041：路由策略分页
- 变更内容：
  - 后端 `GET /api/v1/ai/route/policies` 新增 `offset/limit` 参数，响应增加 `offset/limit/total` 元信息。
  - 控制台“路由策略概览”改为服务端分页拉取，新增“上一页/下一页”与区间计数展示。
  - 保留现有关键字筛选与批量删除能力，并补充后端分页测试。
- 变更原因：策略规模扩大后，避免一次拉全量数据，提升控制台加载与操作可控性。
- 影响范围：AI 路由策略列表接口返回结构扩展；前端策略面板分页交互增强。

### CHG-042：路由策略批量更新禁用模型
- 变更内容：
  - 后端新增 `POST /api/v1/ai/route/policies/batch-update-disabled-models`，按 `ids[]` 批量更新 `disabled_models`。
  - 控制台策略面板新增“批量设置禁用模型”输入与操作按钮，支持选中多条策略一键更新。
  - 新增后端测试覆盖批量更新链路。
- 变更原因：减少逐条编辑策略时的重复操作，提升运营批量维护效率。
- 影响范围：后端路由策略管理接口增强；前端批量操作能力增强。

### CHG-043：路由策略 JSON 导入导出
- 变更内容：
  - 后端新增 `GET /api/v1/ai/route/policies/export` 与 `POST /api/v1/ai/route/policies/import`。
  - 导入接口支持 `overwrite_existing` 开关，可选择覆盖或跳过已存在策略。
  - 控制台新增导入/导出区域：支持导出 JSON、粘贴 JSON 导入、覆盖开关控制。
  - 新增后端测试覆盖导出与导入（新增+覆盖）流程。
- 变更原因：支持策略备份、迁移和批量初始化，降低跨环境配置成本。
- 影响范围：后端策略管理接口扩展；前端运营工具能力增强。

### CHG-044：支付渠道接口增强（回调验签与退款占位）
- 变更内容：
  - 扩展 `POST /api/v1/billing/checkout` 响应：增加 `provider_order_no`、`callback_url`、`callback_verify_required`、`refund_supported` 字段。
  - 新增 `POST /api/v1/billing/checkout/callback`（占位）：支持上传渠道回调数据与签名字段，先以占位规则完成验签结果判定与订单状态流转。
  - 新增 `POST /api/v1/billing/refund`（占位）：支持按订单发起退款申请并返回受理状态，后续对接真实支付宝/微信退款网关。
  - 新增测试覆盖：下单扩展字段、回调验签成功/失败、退款占位流程。
- 变更原因：提前对齐支付宝/微信联调所需的关键接口字段与状态流，降低后续真网关接入改造成本。
- 影响范围：后端 billing API 契约扩展；前端与运营可基于新字段提前联调支付与退款流程。

### CHG-045：示例插件调用统一走 AI 记账链路
- 变更内容：
  - `POST /api/v1/plugins/use` 在 `api_name=ai:invoke` 时，不再直接调用网关底层方法，而是复用 `/api/v1/ai/invoke` 同一条执行链路。
  - `UsePluginRequest` 增加可选 `payload` 字段，允许插件动作在运行时附加输入。
  - 新增测试覆盖：插件触发 AI 调用后，`/api/v1/ai/usage/summary` 调用计数增长。
- 变更原因：避免插件调用绕过配额/审计/计费逻辑，确保“插件入口”和“AI 入口”账务一致。
- 影响范围：后端插件运行路径增强；AI 用量统计与计费一致性提升。

### CHG-046：全量开发收口基线与自动检测
- 变更内容：
  - 新增 `docs/memory/full-delivery-baseline.md`，固化范围基线、里程碑标记、终态门槛与业务验收入口。
  - 新增 `scripts/checkpoint.ps1`，支持 `milestone/final` 两种自动检查模式（后端测试 + 前端构建 + 终态状态检查）。
  - 已执行一次里程碑检测并通过，作为后续“大步推进”默认节奏。
- 变更原因：将“连续开发+阶段可行性验证”流程产品化，减少遗漏与回归风险。
- 影响范围：研发交付流程增强，便于持续推进后统一业务验收。
